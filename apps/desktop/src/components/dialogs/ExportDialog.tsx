...
export function ExportDialog() {
  ...
  async function exportPng() {
    if (!globalStageRef) throw new Error('Canvas not ready')
    const stage = globalStageRef

    if (range === 'card') {
      const dataUrl = stage.toDataURL({ pixelRatio: resolution.pixelRatio })
      const cardName = 'card'
  ...
  async function exportPdf() {
    ...
    const dataUrl = stage.toDataURL({ pixelRatio: resolution.pixelRatio })
    const allUrls = range === 'card' ? [dataUrl] : Array(cards.length).fill(dataUrl)
